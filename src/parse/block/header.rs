    ...
    trace(
        ...
        move |input: &mut Input| {
            ...
            loop {
                ...
                if !indent_level.is_more_indented_than(spaces) {
                    break;
                }
...
mod tests {
    ...
    /// same threshold is, correctly, the right one). `IndentLevel::is_more_indented_than` compares
    /// against the unsaturated internal representation instead, so it tells the two apart.
    #[test]
