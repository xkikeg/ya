...
The phase-by-phase implementation log for [`ya`](../../README.md), from the first parser skeleton
through the 0.4.0 release. It is kept because the writeups here explain decisions the code alone
doesn't: every deviation from the original plan, every root cause behind a latent bug, and every
"we tried X first and it was wrong" that the finished source no longer shows.

**This is a record, not a plan.** Every item below is done. Current guidance -- architecture,
conventions, the spec-first playbook, and what's still open -- lives in
[`AGENT.md`](../../AGENT.md); read that first. Come here when you want to know *why* something is
the way it is, or when a bug looks familiar and you want the earlier writeup on it.

Phases landed roughly in order, each mostly depending only on earlier ones. Future phases get
appended here once they land.

## Phases 0-10 (through 0.4.0)
...
These were open questions raised to the maintainer while the phases above were in flight, kept with
their original numbering. Question 1 (alias eager substitution) is still open and lives in
[`AGENT.md`](../../AGENT.md#open-items).
