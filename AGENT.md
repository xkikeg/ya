...
- [x] **Zero-indented block scalar at the document root** (`DK3J`, `FP8R`, both `--- >` with
      ...
      Fixed by adding `IndentLevel::is_more_indented_than(spaces)`, which compares against the
      unsaturated internal `n + 1` representation instead of the lossy `get()`, and switching
      `detect_indentation` to use it. This also uncovered a **pre-existing, unrelated test bug**:
